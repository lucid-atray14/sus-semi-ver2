    ...
    if selected_page == "Home":
        ...
        with st.expander("ℹ️ Bandgap Reference", expanded=False):
            ...
            *The shaded regions represent these energy ranges.*
            ...
            # Define the improved filter function here
            def filter_dataframe_custom(df, filters, selected_names=None):
                """Filter dataframe based on provided filters and optional names"""
                filtered = df.copy()
            ...
            if st.session_state.filters:  # Check if any filters exist
                ...
                filtered_df = filter_dataframe_custom(df, st.session_state.filters)
                ...
                if not filtered_df.empty:
                    ...
                    if st.session_state.show_additional_filters:
                        ...
                        with st.expander("🔧 Choose 2 Additional Filters for Second Graph", expanded=True):
                            ...
                            # Use a form to prevent rerun on every slider change
                            with st.form(key="second_graph_form"):
                                cols = st.columns(2)
                                
                                with cols[0]:
                                    filter_1 = st.selectbox("First filter (X-axis)", available_filters, key="filter_1_select_form")
                                    if filter_1:
                                ...
                                with cols[1]:
                                    ...
                                    filter_2 = st.selectbox("Second filter (Y-axis)", available_for_filter_2, key="filter_2_select_form")
                                    if filter_2:
                        ...
                        if 'show_second_graph' in st.session_state and st.session_state.show_second_graph:
                            ...
                            filtered_df_2 = filter_dataframe_custom(df, st.session_state.filters)
